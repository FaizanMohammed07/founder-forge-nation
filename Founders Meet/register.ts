...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    if (event_slug === "founders-meet-2026" && !designation) {
      return new Response(
        JSON.stringify({
          error: "Designation / year is required for Founders Meet",
        }),
        { status: 400, headers: { "Content-Type": "application/json" } },
      );
    }
    ...
    const totalDuration = Date.now() - startTime;
    console.log(
      `[register] ✓ Registration complete: ${registrationId} (${totalDuration}ms)`,
    );
    ...
    if (duration >= 9000) {
      ...
      return new Response(
        JSON.stringify({
          ...
          message:
            "Registration is processing. Please check your email shortly.",
        }),
